...
{:else}
    <Drawer.Root open={isOpen} onOpenChange={(open) => !open && onClose()}>
        <Drawer.Content
            class="bg-white dark:bg-surface-dark border-t border-slate-100 dark:border-slate-800 rounded-t-[10px] fixed bottom-0 left-0 right-0 z-50 flex flex-col h-[92vh]"
        >
            <div
